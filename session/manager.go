...
func NewManager(cookieName string, maxLifeTime int64) *Manager {
	mgr := &Manager{mCookieName: cookieName, mMaxLifeTime: maxLifeTime, mSessions: make(map[string]*Session)}
...
//在开始页面登陆页面，开始Session
func (mgr *Manager) StartSession(w http.ResponseWriter, r *http.Request) string {
	mgr.mLock.Lock()
...
//结束Session
func (mgr *Manager) EndSession(w http.ResponseWriter, r *http.Request) {
...
//结束session
func (mgr *Manager) EndSessionBy(sessionID string) {
	mgr.mLock.Lock()
	defer mgr.mLock.Unlock()
...
//设置session里面的值
func (mgr *Manager) SetSessionVal(sessionID string, key interface{}, value interface{}) {
	mgr.mLock.Lock()
	defer mgr.mLock.Unlock()
...
//得到session里面的值
func (mgr *Manager) GetSessionVal(sessionID string, key interface{}) (interface{}, bool) {
	mgr.mLock.RLock()
...
//得到sessionID列表
func (mgr *Manager) GetSessionIDList() []string {
	mgr.mLock.RLock()
	defer mgr.mLock.RUnlock()
...
func (mgr *Manager) CheckCookieValid(w http.ResponseWriter, r *http.Request) string {
...
//更新最后访问时间
func (mgr *Manager) GetLastAccessTime(sessionID string) time.Time {
	mgr.mLock.RLock()
	defer mgr.mLock.RUnlock()
...
//创建唯一ID
func (mgr *Manager) NewSessionID() string {
	b := make([]byte, 32)
